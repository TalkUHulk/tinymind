...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None, tokenizer=None):
    ...
    for step, (X, Y, loss_mask, pixel_values) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            ...
            decoded_text = decoded_text.replace("@" * 196, "<image>")
            print(decoded_text, '\n\n')
...
if __name__ == "__main__":
    ...
    vlm_config =  VLMConfig(ve_hidden_size=256, hidden_size=512, num_hidden_layers=8, use_moe=False,
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb.init(project=args.wandb_project, name=wandb_run_name, id=wandb_id, resume=resume, mode="offline", logdir="./swanlog/tiny_s")
    
    # ========== 5. 定义模型、数据、优化器 ==========
    model, tokenizer, preprocess = init_tiny_vlm_model(vlm_config, 
                                                   device=args.device, tokenizer_path="/lh/hulk/MiniMind2-Small", 
                                                   vision_model_type="TinyCLIP-ViT-8M-16-Text-3M",
                                                   vision_model_path='/lh/hulk/Cream-0ef394cd0c0f41b55fb073ab9abbb95acc13104e/TinyCLIP-ViT-8M-16-Text-3M-YFCC15M.pt', 
                                                   )

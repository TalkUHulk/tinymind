...
class MiniMindConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        self.flash_attn = flash_attn if torch.cuda.is_available() else False
        ####################################################
        # Here are the specific configurations of MOE
        # When use_moe is false, the following is invalid
        ####################################################
...
class AttentionExport(nn.Module):
    def __init__(self, args: MiniMindConfig):
        super().__init__()
        self.num_key_value_heads = args.num_attention_heads if args.num_key_value_heads is None else args.num_key_value_heads
        assert args.num_attention_heads % self.num_key_value_heads == 0
    ...
    def forward(self,
        ...
        xq, xk = apply_rotary_pos_emb(xq, xk, position_embedding_cos[:seq_len], position_embedding_sin[:seq_len])
...
class MiniMindBlockExport(nn.Module):
    def __init__(self, layer_id: int, config: MiniMindConfig):
        super().__init__()
        self.num_attention_heads = config.num_attention_heads
        self.hidden_size = config.hidden_size
        self.head_dim = config.hidden_size // config.num_attention_heads
        self.self_attn = AttentionExport(config)
    ...
    def forward(self, hidden_states, attention_mask,
                cos_position_embeddings, sin_position_embeddings,
                past_key=None, past_value=None, use_cache=False):
        residual = hidden_states

        hidden_states, present_key, present_value = self.self_attn(
            self.input_layernorm(hidden_states),
            position_embedding_cos=cos_position_embeddings,
            position_embedding_sin=sin_position_embeddings,
            past_key=past_key, past_value=past_value, use_cache=use_cache, attention_mask=attention_mask
        )
        hidden_states += residual
        hidden_states = hidden_states + self.mlp(self.post_attention_layernorm(hidden_states))
...
class MiniMindModelExport(nn.Module):
    def __init__(self, config: MiniMindConfig):
        super().__init__()
        self.config = config
        self.vocab_size, self.num_hidden_layers = config.vocab_size, config.num_hidden_layers
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size)
        self.dropout = nn.Dropout(config.dropout)
        self.layers = nn.ModuleList([MiniMindBlockExport(l, config) for l in range(self.num_hidden_layers)])
        self.norm = RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(self,
        ...
        for layer_idx, layer in enumerate(self.layers):
            hidden_states, present_key, present_value = layer(
                hidden_states,
                cos_position_embeddings,
                sin_position_embeddings,
                past_keys=past_keys[0].unsqueeze(0),
                past_values=past_values[0].unsqueeze(0),
                use_cache=use_cache,
                attention_mask=attention_mask
            )
            presents_key.append(present_key)
            presents_value.append(present_value)

        hidden_states = self.norm(hidden_states)

        return hidden_states, torch.cat(presents_key, 0), torch.cat(presents_value, 0)
...
class MiniMindForCausalLMExport(PreTrainedModel, GenerationMixin):
    config_class = MiniMindConfig

    def __init__(self, config: MiniMindConfig = None):
        self.config = config or MiniMindConfig()
        super().__init__(self.config)
        self.model = MiniMindModelExport(self.config)
        self.lm_head = nn.Linear(self.config.hidden_size, self.config.vocab_size, bias=False)
        self.model.embed_tokens.weight = self.lm_head.weight
        self.OUT = CausalLMOutputWithPast()

...
class VisualTransformer(nn.Module):
    ...
    def last_hidden_state(self, x: torch.Tensor,
                hidden_z: Optional[torch.Tensor] = None,
                heads_z: Optional[torch.Tensor] = None,
                mha_z: Optional[torch.Tensor] = None,
                intermediate_z: Optional[torch.Tensor] = None,
...
class ImageEncoder(nn.Module):
    ...
    def last_hidden_state(self, image, normalized=False):
        mask = {}
        if self.l0_module is not None:
            mask = self.l0_module.forward()
        ...
        return self.visual.last_hidden_state(image, **mask)
...
class CLIP(CLIPBase):
    ...
    def last_hidden_state(self, image, normalized=False):
        with self.image_autocast():
            return self._image_encoder.last_hidden_state(image, normalized=normalized)
